...
public class CloseUserContextAction {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response, UCH uch)
		...
		closeHttpSession(request, uch);
	}
	
	/**
	 * Close the session bound with specified request.
	 * 
	 * @param request an object of HttpServletRequest
	 * @param uch an object of UCH
	 */
	private void closeHttpSession(HttpServletRequest request, UCH uch) {
		
		if ( request == null )
			return;
		
		uch.closeUserContext( request );
	}
}
